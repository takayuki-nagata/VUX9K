...
| `0x2000_0000` - `0x2000_0FFF` | 4 KB | **Data RAM** | `.data`, `.bss`, stack, and heap region |
...
The 16-byte boot header occupies byte offsets `0x00` to `0x0F` of Sector 64 (`struct.pack("<IIII", ...)`), followed immediately by machine code:

| Offset | Field | Type | Value / Meaning |
|:---|:---|:---|:---|
| `0x00` - `0x03` | Magic Number | `u32` (LE) | `0x56555839` (`"VUX9"` ASCII) |
| `0x04` - `0x07` | ISA Mode | `u32` (LE) | `0` = Hack 16-bit, `1` = RISC-V RV32I |
| `0x08` - `0x0B` | Binary Byte Length | `u32` (LE) | Exact size of executable binary in bytes |
| `0x0C` - `0x0F` | Reserved / Flags | `u32` (LE) | Reserved (`0x0000_0000`) |
| `0x10` - `0x1FF` | Payload (Sector 64 Start) | `bytes` | First chunk of binary machine code (up to 496 bytes) |

Subsequent 512-byte sectors continue the payload until the full binary length is loaded.
...
# 4. Inspect Sector 64 Header & verify Magic/Mode/Size
...
| Inspect SD | `s` | Parses Sector 64 boot header (Magic, ISA Mode, Binary Size) |
| Write SD | `w` | Receives multi-sector binary stream over UART and flashes to Sector 64+ |
| Boot | `l` | Loads Sector 64 payload into Instruction RAM and jumps to execution |
...
| Reset | `r` | Software resets the SoC back to address `0x0000_0000` |
...
| **Real Hardware** | `make test-hw` | ~25 sec | Automated physical hardware execution on Tang Nano 9K via `scripts/test_hardware.py` (8/8 PASS 100%) |
